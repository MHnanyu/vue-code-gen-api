...
GENERATION_PROMPT_PREFIX = """【原型极简原则】
- 这是一次性原型图，不是生产代码
- 必须且只能生成 MainPage.vue 一个文件（文件名必须为 MainPage.vue，不要使用其他名称如 LoginPage、DashboardPage 等）
...
def get_agent_generation_prompt(component_lib: str, requirement_doc: str) -> str:
    return GENERATION_PROMPT_PREFIX + "\n" + requirement_doc


def get_agent_optimization_prompt(component_lib: str) -> str:
    return OPTIMIZATION_PROMPT_PREFIX
